module SchoolNameMatch # consider using official rails concern
  MATCHES = [
  ...
  def self.find_by_school_name(school_name)
    MATCHES.find{|m| m[:school_name] == school_name && m[:match_confidence] >= MIN_MATCH_CONFIDENCE}
  end

  # def matched_names
  #   MATCHES.select{|m| m[:matching_uuid] == uuid && m[:match_confidence] >= MIN_MATCH_CONFIDENCE}
  # end
end
